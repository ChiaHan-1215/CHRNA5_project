...
################################################
######## Update: 12192024 test Isoform_inputcle ########
################################################
tissue_list_files <- list.files('/Volumes/ifs/DCEG/Branches/LTG/Prokunina/GTEx_data/CHRNA5_project/Recount3_GTEx_junction_count_table/CHRNA5_6_isofoms_jc_with_GT_smoking/')
# Remove Brain tissue 
# remove FALLOPIAN_TUBE
# remove BONE MARROW as NO DATA
# remove Bladder as it's already done
tissue_list_files <- tissue_list_files[-c(3,6,7,12)]


df.out_all <- data.frame()

for (x in tissue_list_files){
  #x <- tissue_list_files[6]
  
  Isoform_input <- read.csv(paste0('/Volumes/ifs/DCEG/Branches/LTG/Prokunina/GTEx_data/CHRNA5_project/Recount3_GTEx_junction_count_table/CHRNA5_6_isofoms_jc_with_GT_smoking/',x),header = T)
  Isoform_input <- Isoform_input[,c(1,2,16:22)]
  

  # Now calculating the ratio of isofoms
  for (n in grep("V416",names(Isoform_input),value = T)){
    # n <- grep("V416",names(Isoform_input),value = T)[1]
    Isoform_input[[paste0(n,'_rat')]] <- round(Isoform_input[[n]] / Isoform_input$iso_sum,3)
  }
  # change NaN to NA or other text
  Isoform_input <- Isoform_input %>% mutate(across(everything(), ~ifelse(is.nan(.), "sum_is_0", .)))
  ...
  # GTEX sample info for skecetal Isoform_inputcle
  ...
  gender_sp <- c("Ovary" , 'Uterus' , 'Testis' ,  'Vagina',
                 "Prostate" , "Cervix.Ectocervix","Fallopian.Tube","Cervix.Endocervix")
  
  
  

  set1 <- left_join(Isoform_input,samid,by='full_id')
  ...
  set1 <- set1 %>% filter(tissue != "Cells.EBV-transformed.lymphocytes")
  
  #########Not here?
  # Get TPM 
  tissue_list_keys <- (set1$tissue) %>% unique()
  df_f_Isoform_input <- df_f[,c(1:98,grep(paste0("CHRNA5.*geneTPM_(", paste(tissue_list_keys, collapse = "|"), ")"), names(df_f),ignore.case = T))]
  # check
  names(df_f_Isoform_input)
  #########
  
  
  
  set1 <- left_join(set1,df_f_Isoform_input,by='GTEx_ID')
  ...
  for (k in tissue){
    ...
    # TPM >0 within tissue k, BUT STILL KEEP NA since some of them is missing data not actually 0!!
    ssd <- df_sub %>%
      filter(tissue == k & .data[[grep(k, names(df_sub), value = TRUE)]] > 0| is.na(.data[[grep(k, names(df_sub), value = TRUE)]]))
    
    
    # then filter CHRAN5_sum >=2
    
    ssd <- ssd %>% filter(iso_sum >= 2)
    ...
    for (j in selected_col){
      ...
      ####################################################
      # 01022025: the lm analysis of smoking vs isoform  #
      ####################################################
      
      # For C5 smoking analysis, use "smoke_Current_vs_No.Former"
      # iso~smoking + SEX + AGE without SNPs???
      ...
      if(any(gender_sp %in% k)){
        ...
        fmla_inter_set3 <- as.formula(paste(j, "~", "smoke_Current_vs_No.Former + AGE"))
        fmla_inter_set2 <- as.formula(paste(j, "~", "smoke_Current.Former_vs_Never + AGE"))
        ...
        fmla_inter_set3 <- as.formula(paste(j, "~", "smoke_Current_vs_No.Former + SEX + AGE"))
        fmla_inter_set2 <- as.formula(paste(j, "~", "smoke_Current.Former_vs_Never  + SEX + AGE"))
      ...
      formula_list <- list(fmla_inter_set2,fmla_inter_set3)
      ...
      for (current_formula in formula_list) {
        ...
        tmp.snp.0a <- tmp.snp[ which(tmp.snp[,vars[2]] == 0), ]
        tmp.snp.0b <- as.character("code_as_0")
        tmp.snp.1a <- tmp.snp[ which(tmp.snp[,vars[2]] == 1), ]
        tmp.snp.1b <- as.character("code_as_1")
        ...
        tmp.df <- data.frame(
          ...
          model = paste(deparse(current_formula), collapse = " "),
          dataset = 'all_data',
